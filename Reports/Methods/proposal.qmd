...
Alternatively, or in addition to, we could use the shortest cognitive task FPT as a control, which is expected to take 12 minutes of cognitive testing, and overall would have about the same expected total time as the new short-FPT versions. If we only use this shortest-original-FPT as the control, we won't have a direct comparison with how it would have performed at capturing forecasting accuracy on the items / at that time as the rest of the shortened versions, which seems not totally ideal, but also maybe isn't necessarily super important?
:::
