...
v5test_plt <- test |>
  ...
  annotate("text", label = paste0("10 minutes:\n• ",
                                  test |> filter(time < 10.49) |>
                                    count(task) |>
                                    pmap_chr(\(task, n) paste(n, task)) |> 
                                    paste(collapse = "\n• ")),
           x = 3.3, y = 0.2, size = 3.4, hjust = "left", color = "gray20") +
  annotate("text", label = paste0(filter(test, test_r2 == max(test_r2)) |>
                                  ...
                                  test |>
                                    ...
                                    pmap_chr(\(task, n) paste(n, task)) |> 
                                    paste(collapse = "\n• ")),
           x = 18.5, y = 0.25, size = 3.4, hjust = "left", color = "gray20") +
  
  coord_cartesian(xlim = c(0, 30), ylim = c(0, 0.8), expand = c(0, 1)) +
